...
void __attribute__((picinterrupt(("")))) interrupt_CAD(void) {
    TMR0 = 157;
    if (cont == 1000) {
...
void SK9822_SendColor(uint8_t brigthness, uint8_t red, uint8_t green, uint8_t blue, int n) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        spi_write_read((char) blue);
        spi_write_read((char) green);
        spi_write_read((char) red);
    }
...
void main(void) {
    ...
    while (1) {
        SK9822_SendColor(31, 255, 255, 255, 10);
        printf("NIVEL RUIDO: %d\r\n", NR);
        printf("HUMEDAD: %d\r\n", humity);
